...
import DivStyled from '../styles/ProductCardStyles';
import LinkNoStyles from '../styles/LinkNoStyles';
import { addToCart, selectProductProps } from '../actions/index';
...
class ProductCard extends Component {
  ...
  onAddToCartClick = (e) => {
    ...
    const { attributes, id, addToCart, history } = this.props;

    const attributesNames = attributes.map((attr) => attr.name);
    const obj = {};
    attributesNames.forEach((attr) => {
      obj[attr] = { attributeName: attr, attributeValue: 1 };
    });
    addToCart(id, obj);
    history.push('/cart');
  };
  ...
  render() {
    const { id, name, gallery, prices, currency } = this.props;
      ...
      <LinkNoStyles to={`/product/${id}`}>
        <DivStyled img={gallery[0]}>
          <li src={gallery[0]}>
            ...
            <div onClick={this.onAddToCartClick}>
              <svg
                width="20"
                height="20"
                viewBox="0 0 20 20"
...
export default connect(mapStateToProps, { addToCart, selectProductProps })(
  ProductCard
);
